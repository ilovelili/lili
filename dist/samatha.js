...
(function samanthaScope() {
    ...
    function mul(note, interval) {
        if (typeof interval === 'number') return [note[0] * interval, note[1] * interval];
    ...
    var intervalFromFifth = ['second', 'sixth', 'third', 'seventh', 'fourth', 'unison', 'fifth'];

    var intervalsIndex = ['unison', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'octave', 'ninth', 'tenth', 'eleventh', 'twelfth', 'thirteenth', 'fourteenth', 'fifteenth'];
  ...
  var kNoteIndex = ['c', 'd', 'e', 'f', 'g', 'a', 'b'];
    ...
    // samantha.note namespace - All notes should be instantiated
    // through this function.
    samantha.note = function(name, duration) {
        if (typeof name === 'string') return samantha.note.fromString(name, duration);
        else return new SamanthaNote(name, duration);
    };

    samantha.note.fromKey = function(key) {
        ...
        return samantha.note(diff ? add(note, mul(sharp, diff)) : note);
    };

    samantha.note.fromFrequency = function(fq, concertPitch) {
        ...
        return {
            note: samantha.note.fromKey(key),
            cents: cents
        };
    };

    samantha.note.fromMIDI = function(note) {
        return samantha.note.fromKey(note - 20);
    };

    samantha.note.fromString = function(name, dur) {
        ...
        if (parser && name === parser[0] && parser[3].length) {
            ...
            else if (octave.match(/^'+$/) && lower) octave = 3 + octave.length;
            else if (octave.match(/^,+$/) && !lower) octave = 2 - octave.length;
        ...
        return new SamanthaNote(sub(note, A4), dur);
    };

    // samantha.chord namespace - All chords should be instantiated
    // through this function.
    samantha.chord = function(name, symbol) {
        if (typeof name === 'string') {
            var root, octave;
            root = name.match(/^([a-h])(x|#|bb|b?)/i);
            if (root && root[0]) {
                octave = typeof symbol === 'number' ? symbol.toString(10) : '4';
                return new SamanthaChord(samantha.note(root[0].toLowerCase() + octave),
                name.substr(root[0].length));
            }
        }
        else if (name instanceof SamanthaNote) return new SamanthaChord(name, symbol);
     ...
     */
    samantha.interval = function(from, to) {
        // Construct a SamanthaInterval object from string representation
        if (typeof from === 'string') return samantha.interval.toCoord(from);

        if (typeof to === 'string' && from instanceof SamanthaNote) return samantha.interval.from(from, samantha.interval.toCoord(to));

        if (to instanceof SamanthaNote && from instanceof SamanthaNote) return samantha.interval.between(from, to);

        throw new Error('Invalid parameters');
    };

    samantha.interval.toCoord = function(simple) {
        ...
        if ((type === 'perfect' && (quality === 'M' || quality === 'm')) || (type === 'minor' && quality === 'P')) {
        ...
        return new SamanthaInterval(coord);
    };

    /**
     * Returns the note from a given note (from), with a given interval (to)
     */
    samantha.interval.from = function(from, to) {
        return new SamanthaNote(add(from.coord, to.coord));
    };

    /**
     * Returns the interval between two instances of samantha.note
     */
    samantha.interval.between = function(from, to) {
        return new SamanthaInterval(sub(to.coord, from.coord));
    };

    samantha.interval.invert = function(sInterval) {
        return samantha.interval(sInterval).invert().toString();
    };

    // samantha.scale namespace - Scales are constructed through this function.
    samantha.scale = function(tonic, scale) {
        if (!(tonic instanceof SamanthaNote)) tonic = samantha.note(tonic);

        return new SamanthaScale(tonic, scale);
    };

    samantha.scale.scales = {};

    function SamanthaNote(coord, duration) {
      duration = duration || {};
    ...
    SamanthaNote.prototype = {
      ...
      /**
       * Sugar function for samantha.scale(note, scale)
       */
      scale: function(scale) {
        return samantha.scale(this, scale);
      },
    
      /**
       * Sugar function for samantha.interval(note, interval)
       */
      interval: function(interval) {
        return samantha.interval(this, interval);
      },
    
      /**
       * Transposes the note, returned by SamanthaNote#interval
       */
      transpose: function(interval) {
        var note = samantha.interval(this, interval);
       ...
       * if (samantha.note('a').scaleDegree(samantha.scale('a', 'major'))) {
       *   ...
       * }
       *
    ...
    function SamanthaInterval(coord) {
      this.coord = coord;
    }
    
    SamanthaInterval.prototype = {
      name: function() {
    ...
    function SamanthaChord(root, name) {
        name = name || '';
        ...
        var i, length, c, shortQ, parsing = 'quality',
            additionals = [],
        ...
        this.intervals = notes.slice(0, chordLength + 1).concat(additionals).map(function(i) {
            return samantha.interval(i);
        });
        ...
        if (bass) {
            ...
            note = samantha.note(bass + (root.octave() + 1));

            bassInterval = samantha.interval.between(root, note);
            ...
            for (i = 0; i < length; i++) {
                if (intervals[i].simple() === bass) continue;
    ...
    SamanthaChord.prototype = {
        notes: function() {
            var voicing = this.voicing(),
                notes = [];

            for (var i = 0, length = voicing.length; i < length; i++) {
                notes.push(samantha.interval.from(this.root, voicing[i]));
            }
        ...
        voicing: function(voicing) {
            ...
            this._voicing = [];
            for (var i = 0, length = voicing.length; i < length; i++) {
                this._voicing[i] = samantha.interval(voicing[i]);
            }
        ...
        dominant: function(additional) {
            ...
            return new SamanthaChord(this.root.interval('P5'), additional);
        },

        subdominant: function(additional) {
            additional = additional || '';
            return new SamanthaChord(this.root.interval('P4'), additional);
        },
        ...
        parallel: function(additional) {
            ...
            if (quality === 'major') {
                return new SamanthaChord(this.root.interval('m3', 'down'), 'm');
            }
            else {
                return new SamanthaChord(this.root.interval('m3', 'up'));
            }
        },
            ...
            if (typeof interval === 'string' && interval in kStepNumber) {
                ...
                for (i = 0, length = intervals.length; i < length; i++) {
                    if (intervals[i].number() === interval) {
                        return samantha.interval.from(this.root, intervals[i]);
                    }
                }
        ...
        interval: function(interval) {
            return new SamanthaChord(this.root.interval(interval), this.symbol);
        },
    ...
    samantha.scale.scales.ionian = samantha.scale.scales.major =
      ['P1', 'M2', 'M3', 'P4', 'P5', 'M6', 'M7'];
    samantha.scale.scales.dorian = ['P1', 'M2', 'm3', 'P4', 'P5', 'M6', 'm7'];
    samantha.scale.scales.phrygian = ['P1', 'm2', 'm3', 'P4', 'P5', 'm6', 'm7'];
    samantha.scale.scales.lydian = ['P1', 'M2', 'M3', 'A4', 'P5', 'M6', 'M7'];
    samantha.scale.scales.mixolydian = ['P1', 'M2', 'M3', 'P4', 'P5', 'M6', 'm7'];
    samantha.scale.scales.aeolian = samantha.scale.scales.minor =
      ['P1', 'M2', 'm3', 'P4', 'P5', 'm6', 'm7'];
    samantha.scale.scales.locrian = ['P1', 'm2', 'm3', 'P4', 'd5', 'm6', 'm7'];
    samantha.scale.scales.majorpentatonic = ['P1', 'M2', 'M3', 'P5', 'M6'];
    samantha.scale.scales.minorpentatonic = ['P1', 'm3', 'P4', 'P5', 'm7'];
    samantha.scale.scales.chromatic = samantha.scale.scales.harmonicchromatic =
      ['P1', 'm2', 'M2', 'm3', 'M3', 'P4', 'A4', 'P5', 'm6', 'M6', 'm7', 'M7'];


    samantha.SamanthaNote = SamanthaNote;
    samantha.SamanthaChord = SamanthaChord;
    samantha.SamanthaScale = SamanthaScale;
    samantha.SamanthaInterval = SamanthaInterval;

    if (typeof exports !== 'undefined') {
        if (typeof module !== 'undefined' && module.exports) exports = module.exports = samantha;

        exports.samantha = samantha;
    }
    else if (typeof this !== 'undefined') this.samantha = samantha;
    else if (typeof window !== 'undefined') window.samantha = samantha;
})();
